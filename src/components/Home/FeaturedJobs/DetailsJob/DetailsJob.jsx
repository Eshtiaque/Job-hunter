...
const DetailsJob = ({ job }) => {
  ...
  return (
    <div className="border p-5 space-y-2 relative">
      ...
      <span className="btn-outline-black rounded p-1 text-white bg-pink-500">{work_type}</span>{" "}
      <span className="btn-outline-black rounded p-1 text-white bg-pink-500">{worktime}</span>
      ...
      <Link to={`/details/${id}`}>
        <button
          onClick={() => navigate(id)}
          className='font-bold text-white p-2 rounded-md bg-pink-500'>
          View Details
        </button>
      </Link>
    </div>
