  ...
  return (
    <div className="my-container">
      <div className="flex justify-end my-5 mr-3">
        <select
          className="bg-pink-500  text-white p-2 rounded-md outline-none border-none"
        ...
        {isFilter.map((job) => (
          <div
            ...
            <div className="lg:space-y-2 space-y-1">
              <h1 className="lg:text-xl text-sm font-semibold text-black-700 ">
                {job?.position}
              </h1>
              <h1 className="lg:text-md text-xs text-gray-500 font-medium">
                {job?.company}
              </h1>
              <button className="btn-outline-black rounded p-1 text-white bg-pink-500">{job?.work_type}</button>{" "}
              <button className="btn-outline-black rounded p-1 text-white bg-pink-500">{job?.worktime}</button>
            ...
            <div className="ml-auto">
              <Link to={`/details/${job?.id}`}>
                <button className="font-bold text-white p-2 rounded-md bg-pink-500">View Details</button>
              </Link>
            </div>
          </div>
        ))}
